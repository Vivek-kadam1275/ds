...
public class Ring {
    ...
    void initElection(int process_id) {
        ...
        // System.out.println("Process P" + process_id + " has declared P" + coordinator
        // + " as the coordinator");
        // pid.clear();
        // }
        if (!processes[process_id - 1]) {
        ...
        prev = (process_id - 1) % max_processes;

        System.out.println("Process P" + process_id + " is initiating election...");
        ...
        } while (current != prev); // ensures full ring is covered

        if (processes[prev]) { // include the initiator if alive
            pid.add(prev + 1);
        }

        Collections.sort(pid);
        int len = pid.size();
        coordinator = pid.get(len - 1);
